...
class TestOptionNames:
    ...
    def test_every_listened_event_means_something(self):
        for event in CLAUDE_STATUS_EVENTS:
            assert event == "SessionEnd" or _commands(event), event
...
class TestMainThread:
    ...
    def test_ask_user_question_waits_through_its_permission_request(self):
        # AskUserQuestion fires PreToolUse and then PermissionRequest.
        assert _state_set(_commands("PreToolUse", tool_name="AskUserQuestion")).startswith(
            "running"
        )
        assert _state_set(_commands("PermissionRequest", tool_name="AskUserQuestion")).startswith(
            "waiting"
        )
...
class TestSubagents:
    ...
    def test_a_permission_request_marks_the_waiter(self):
        commands = _commands("PermissionRequest", agent_id="a4b6")
        assert commands == [
            ["set-option", "-p", "-t", PANE, AGENT_WAITER_OPTION, f"a4b6 {NOW:.3f}"]
        ]
